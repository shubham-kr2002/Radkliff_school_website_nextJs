 ...
 *  Get one free at https://web3forms.com (no credit card). */
...
export default function EnquirySection() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    if (!ACCESS_KEY) {
      setError("Web3Forms key not configured. Contact site admin.");
      return;
    }
    setLoading(true);
    setError("");
    ...
    try {
      ...
      if (res.status === 200) {
        ...
        setError(json.message || "Form submission failed.");
      }
    } catch {
        ...
        <div className="w-full max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-16 relative z-10 items-center justify-center">
          ...
          <div className="lg:col-span-7 order-1 lg:order-2 relative">
            ...
            <div className="relative bg-white/70 backdrop-blur-xl border border-white p-6 md:p-8 rounded-[2rem] md:rounded-[2.5rem] paper-cut-shadow-lg overflow-hidden">
              ...
              {error && (
                <p className="text-xs text-error mb-2 bg-error-container/20 px-4 py-2 rounded-full">{error}</p>
              )}
              ...
              <form className="space-y-3 md:space-y-4" onSubmit={handleSubmit}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4">
                  <div className="space-y-1">
                    ...
                    <input type="tel" name="mobile" pattern="[0-9]{10}" title="10 digit mobile number" placeholder="9876543210" className="w-full bg-surface-container-low border border-outline-variant/50 focus:border-primary px-4 py-2 md:py-2.5 rounded-full font-body-md text-sm text-on-surface focus:outline-none focus:ring-2 focus:ring-primary/20 transition-all shadow-sm" required />
                  </div>
                  <div className="space-y-1">
                    <label className="font-label-sm text-[10px] font-bold text-on-surface-variant uppercase tracking-wider ml-2">Pin Code*</label>
                    <input type="text" name="pin_code" pattern="[0-9]{6}" title="6 digit pin code" placeholder="123456" className="w-full bg-surface-container-low border border-outline-variant/50 focus:border-primary px-4 py-2 md:py-2.5 rounded-full font-body-md text-sm text-on-surface focus:outline-none focus:ring-2 focus:ring-primary/20 transition-all shadow-sm" required />
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4">
                  <div className="space-y-1">
                    ...
                    <input type="text" name="country" placeholder="India" className="w-full bg-surface-container-low border border-outline-variant/50 focus:border-primary px-4 py-2 md:py-2.5 rounded-full font-body-md text-sm text-on-surface focus:outline-none focus:ring-2 focus:ring-primary/20 transition-all shadow-sm" required />
                  </div>
                  <div className="space-y-1">
                    ...
                    <input type="text" name="state" placeholder="Rajasthan" className="w-full bg-surface-container-low border border-outline-variant/50 focus:border-primary px-4 py-2 md:py-2.5 rounded-full font-body-md text-sm text-on-surface focus:outline-none focus:ring-2 focus:ring-primary/20 transition-all shadow-sm" required />
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4">
                  <div className="space-y-1">
                    ...
                    <input type="text" name="city" placeholder="Jaipur" className="w-full bg-surface-container-low border border-outline-variant/50 focus:border-primary px-4 py-2 md:py-2.5 rounded-full font-body-md text-sm text-on-surface focus:outline-none focus:ring-2 focus:ring-primary/20 transition-all shadow-sm" required />
                  </div>
                  <div className="space-y-1">
                    <label className="font-label-sm text-[10px] font-bold text-on-surface-variant uppercase tracking-wider ml-2">Location*</label>
                    <input type="text" name="location" placeholder="Jagatpura" className="w-full bg-surface-container-low border border-outline-variant/50 focus:border-primary px-4 py-2 md:py-2.5 rounded-full font-body-md text-sm text-on-surface focus:outline-none focus:ring-2 focus:ring-primary/20 transition-all shadow-sm" required />
                  </div>
                </div>

                <div className="pt-2">
                  <button type="submit" disabled={loading} className="w-full tactile-button magical-glow bg-[#FFF176] text-[#2C3333] font-bold px-6 py-3 rounded-full border-b-[4px] border-[#FBC02D] text-sm md:text-base hover:-translate-y-1 transition-transform shadow-xl flex items-center justify-center gap-2 disabled:opacity-60 disabled:cursor-not-allowed">
                    {loading ? (
                      <>Sending <span className="material-symbols-outlined text-lg animate-spin">progress_activity</span></>
                    ) : (
